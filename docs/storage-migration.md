...
- **S3 CopyObject** (requires parent R2 credentials: `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, and `R2_ACCOUNT_ID`): Zero data through the Worker — R2 copies the object server-side. Object size is limited only by R2 S3 API limits. Structurally covered (same code path as the streaming fallback up to the `fetch()` call) and SigV4 signing is unit-tested, but **not yet runtime-proven** until the credentialed proof runs.

`scripts/proof-copy.sh` now supports both modes. When parent R2 credentials are configured (via environment variables or macOS Keychain: `pocketflare-r2-access-key-id`, `pocketflare-r2-secret-access-key`, `pocketflare-r2-account-id`), the script derives 15-minute credentials scoped to the `STORAGE` bucket's `proof-copy/` prefix, passes those temporary values into a dedicated `wrangler dev` instance, asserts the correct `copyPath` is reported, and verifies there are no S3 HTTP errors or streaming-fallback warnings in the S3 mode logs. Without parent credentials, the S3 path is skipped with a clear message and only the streaming fallback is proven.
