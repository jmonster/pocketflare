...
Pocketflare uses the Worker relay path for runtime filesystem Copy. A server-side R2 S3 `CopyObject` optimization was tested with scoped R2 credentials, but deployed Workers rejected `r2.cloudflarestorage.com` fetch URLs before any HTTP request reached R2. That path is disabled so optional credentials cannot activate a broken copy path.
...
- **Streaming fallback** (supported): Uses `FixedLengthStream` + `pipeTo` to relay source body to destination. The copy path itself has no Go-side buffering — memory stays bounded regardless of object size. The proof route allocates source generation and verification buffers (capped at 20 MiB), but those are test scaffolding, not part of the copy path under test.
- **S3 CopyObject** (disabled): Tested on June 3, 2026 with temporary credentials scoped to `test/proof-copy/` and disposable deployed Workers. Credential minting, Worker deploy, health, auth, source upload, and cleanup all worked; `fetch("https://<account>.r2.cloudflarestorage.com/<bucket>/<key>")` and virtual-hosted-style `fetch("https://<bucket>.<account>.r2.cloudflarestorage.com/<key>")` both rejected with `TypeError: Invalid URL string` before HTTP. The `test` bucket was empty after each run.

`scripts/proof-copy.sh` proves the supported streaming fallback (1 KiB through 20 MiB). If R2 credentials are present, the script reports that S3 CopyObject is disabled and still runs the fallback proof.
