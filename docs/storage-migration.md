...
`scripts/proof-copy.sh` proves the supported streaming fallback with 1 KiB, 10 MiB, and 20 MiB objects:
...
- **S3 CopyObject** (disabled): Scoped-credential deployed Worker proof rejected R2 S3 endpoint fetches before HTTP, so credentials cannot enable this optimization.
...
Checked-in fixtures are generated by `tests/fixtures/generate-backup-zip.mjs` and `tests/fixtures/generate-large-backup-zip.mjs`. Run `scripts/proof-restore-cli.sh` to verify minimal restore, restore-token resume, and the 1000-record restore path against a local `wrangler dev` Worker.
