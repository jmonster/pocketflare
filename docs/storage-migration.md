...
`scripts/proof-copy.sh` supports both modes. When parent R2 credentials are configured, the script derives 15-minute credentials scoped to the STORAGE bucket's `proof-copy/` prefix, passes those into wrangler dev, asserts the correct `copyPath` is reported, and checks for S3 HTTP errors. Without parent credentials, the S3 path is skipped and only the streaming fallback is proven (17/17 passing, 1 KiB through 20 MiB).
